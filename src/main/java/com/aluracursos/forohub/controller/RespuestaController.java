...
public class RespuestaController {
    ...
    public ResponseEntity registrar(@RequestBody @Valid DatosRegistroRespuesta datos) {
        var topico = topicoRepository.findById(datos.idTopico()).orElseThrow(
                () -> new RuntimeException(("Tópico no encontrado.")));
    ...
    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity actualizar(@PathVariable Long id, @RequestBody @Valid DatosActualizadosRespuesta datos) {
        var respuesta = respuestaRepository.getReferenceById(id);
    ...
    public ResponseEntity marcarComoSolucion(@PathVariable Long id, Principal principal) {
        ...
        if (!loginAutorTopico.equals(usuarioLogueado)) {
            throw new ValidationException("Sólo el autor puede marcarla como solución. ");
        }
